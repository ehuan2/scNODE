...
def basicStats(data, axis="cell"):
    ...
    if axis == "cell":
        ...
        zero_fraction = np.array(
            [len(np.where(cell == 0)[0]) / n_genes for cell in data]
        )
    elif axis == "gene":
        ...
        # fraction of zero
        zero_fraction = np.array(
            [len(np.where(data[:, i] == 0)[0]) / n_cells for i in range(n_genes)]
        )
    else:
...
def _ot_unbalanced(true_data, pred_data, scaling=0.99, blur=0.05, reach=2):
    """
    Compute Wasserstein distance with Sinkhorn algorithm.
    :param true_data (numpy.ndarray): True expression data.
    :param pred_data (numpy.ndarray): Predicted expression data.
...
def globalEvaluationUnbalanced(true_data, pred_data, scaling=0.99, blur=0.05, reach=2):
    """Evaluate the difference between true and reconstructed data at a single time point."""
    assert true_data.shape[1] == pred_data.shape[1]
    l2_dist, cos_dist, corr_dist = _unbalancedDist(true_data, pred_data)
    ot_loss = _ot_unbalanced(
        true_data, pred_data, scaling=scaling, blur=blur, reach=reach
    )
    return {
        "l2": l2_dist,
        "cos": cos_dist,
