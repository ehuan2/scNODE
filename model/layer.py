...
class LinearNet(nn.Module):
    ...
    def __init__(self, input_dim, latent_size_list, output_dim, act_name):
        ...
        if act_name not in ACT_FUNC_MAP:
            raise ValueError(
                "The activation function should be one of {}.".format(
                    ACT_FUNC_MAP.keys()
                )
            )
    ...
    def forwardWithTime(self, t, data):
        ...
        """
        out = self.net(data)
        self.regularization_loss += out.pow(2).mean()
        return out
...
if __name__ == "__main__":
    linear_net = LinearNet(
        input_dim=100, latent_size_list=[64, 32, 16], output_dim=8, act_name="sigmoid"
    )
    print(linear_net)

    linear_net = LinearNet(
        input_dim=100, latent_size_list=None, output_dim=8, act_name="sigmoid"
    )
    print(linear_net)
