...
import numpy as np
from model.layer import LinearVAENet
...
class scNODE(nn.Module):
    ...
    def forward(self, data, tps, batch_size=None):
        ...
        """
        # ensure that the diffeq_decoder's reg. loss is reset:
        self.diffeq_decoder.net.regularization_loss = 0.0
    ...
    def predict(
        self, first_tp_data, tps_to_predict, n_cells
    ):  # NOTE: use first_tp_data instead of first_tp_dist
        """
        ...
        repeat_mean = torch.repeat_interleave(first_latent_mean, repeat_times, dim=0)[
            :n_cells, :
        ]
        repeat_std = torch.repeat_interleave(first_latent_std, repeat_times, dim=0)[
            :n_cells, :
        ]
        ...
        return (
            ...
            latent_seq,
            recon_obs,
        )  # NOTE: also return the list of latent variables
