...
def prep_splits_herring(ann_data, n_tps, data_name, cell_type):
    ...
    print(f'Train tps: {tps_to_continuous(train_tps, times_sorted)}, test tps: {tps_to_continuous(test_tps, times_sorted)}')
    ...
    return cell_ann_data, train_data, train_tps, test_data, test_tps, traj_data, tps, times_sorted

# next we want to prepare the data for training
def prep_splits(ann_data, n_tps, data_name, cell_type):
    if data_name in [Dataset.HERRING, Dataset.HERRING_GABA]:
        return prep_splits_herring(ann_data, n_tps, data_name, cell_type=cell_type)
