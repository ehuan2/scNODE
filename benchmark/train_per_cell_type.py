...
def prep_splits_herring(ann_data, n_tps, data_name, split_type, cell_type):
    ...
    train_tps = [tp for tp in all_tps if tp in train_tps]
    test_tps = [tp for tp in all_tps if tp in test_tps]
    # now we need to make sure there is an overlap
    print(f"Train tps: {train_tps}, test tps: {test_tps}")
    print(
        f"Train tps: {tps_to_continuous(train_tps, times_sorted)}, test tps: {tps_to_continuous(test_tps, times_sorted)}"
    )
    ...
    print("# tps={}, # genes={}".format(tps, n_genes))
...
def prep_splits(ann_data, n_tps, data_name, split_type, cell_type):
    if data_name in [Dataset.HERRING, Dataset.HERRING_GABA]:
        return prep_splits_herring(
            ann_data, n_tps, data_name, split_type, cell_type=cell_type
        )

    raise ValueError(f"Cannot run this functionality...")
    train_tps, test_tps = tpSplitInd(data_name, split_type, n_tps)
    data = ann_data.X
...
def visualize_umap_embeds(
    ...
    true_cell_tps = np.concatenate(
        [np.repeat(tps[t], each.shape[0]) for t, each in enumerate(true_data)]
    )
    pred_cell_tps = np.concatenate(
        [
            np.repeat(tps[t], all_recon_obs[:, t, :].shape[0])
            for t in range(all_recon_obs.shape[1])
        ]
    )
    ...
    if not os.path.exists(vis_embed_checkpoint_path):
        ...
        print(
            f"Successfully saved visualization embeddings to {vis_embed_checkpoint_path}"
        )
        ...
        true_umap_traj = umap_model.transform(
            pca_model.transform(np.concatenate(true_data, axis=0))
        )
        print(
            f"Using the saved visualization embeddings from {vis_embed_checkpoint_path}"
        )
...
def get_umap_embed(all_recon_obs, traj_data, train_data, train_tps):
    ...
    all_cell_tps = np.concatenate(
        [np.repeat(idx, x.shape[0]) for idx, x in enumerate(traj_data)]
    )
    removed_tps = np.concatenate(
        [np.repeat(train_tps[idx], x.shape[0]) for idx, x in enumerate(train_data)]
    )
    aug_cell_tps = np.concatenate(
        [np.repeat(idx, x.shape[0]) for idx, x in enumerate(aug_traj_data)]
    )
    ...
    removed_model_traj = umap_model.transform(
        pca_model.transform(np.concatenate(train_data, axis=0))
    )

    # and finally the enhanced one
    aug_traj = umap_model.transform(
        pca_model.transform(np.concatenate(aug_traj_data, axis=0))
    )
    ...
    return (
        (all_ann_data, true_umap_traj, all_cell_tps),
        (removed_ann_data, removed_model_traj, removed_tps),
        (aug_ann_data, aug_traj, aug_cell_tps),
    )
...
def plot_paga(data, title, save_to):
    ...
    for t_idx, t in enumerate(tps):
        ...
        ax.scatter(
            traj_data[cell_idx, 0],
            traj_data[cell_idx, 1],
            color=color_list[t_idx],
            s=20,
            alpha=1.0,
        )
    ...
    for e in data_edge:
        ax.plot(
            [data_node_pos[e[0]][0], data_node_pos[e[1]][0]],
            [data_node_pos[e[0]][1], data_node_pos[e[1]][1]],
            "k-",
            lw=1.5,
        )
...
def predict_cell_traj(
    all_recon_obs, traj_data, train_data, train_tps, all_tps, data_name
):
    """
    ...
    all_data, removed_data, aug_data = get_umap_embed(
        all_recon_obs, traj_data, train_data, train_tps
    )
...
def train_and_visualize(
    ...
    if args.traj_view:
        print(tps)
        predict_cell_traj(
            all_recon_obs, traj_data, train_data, train_tps, tps, data_name
        )
    ...
    if args.cell_type_to_train == "":
        ...
        if args.cell_type_to_vis != "":
            ...
            recon_obs = scNODEPredict(
                latent_ode_model, cell_traj_data[0], tps, n_cells=n_sim_cells
            )
            visualize_umap_embeds(
                recon_obs,
                cell_traj_data,
        ...
        # now we edit it so that it's only of a specific train subset
        (
            cell_ann_data,
            train_data,
            train_tps,
            ...
            ann_data, n_tps, data_name, split_type=split_type, cell_type=cell_type
        )
